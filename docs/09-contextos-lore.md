...
```mermaid
graph TD
    A["ProvedorContexto (interface)"] --> B["56+ implementações @Component<br/>uma por anime/temporada/filme"]
...
- **Interface:** `ProvedorContexto` (`traducao/domain/ports/`) — `getId()`, `getNomeExibicao()`, `obterPromptSistema()`.
- **Implementações:** classes `@Component` em `traducao/contexto/**`, cobrindo franquias inteiras — ex.: DanMachi (temporadas 1–5 + Orion + Sword Oratoria), toda a linha Gundam (0079, SEED, Zeta, ZZ, Narrative...), Macross (18 variantes), Evangelion (5), Knights of Sidonia, Guilty Crown, 86.
...
1. Crie uma classe em `src/main/java/org/traducao/projeto/traducao/contexto/` implementando `ProvedorContexto`.
...
O painel **"8. Revisão de Lore"** da SPA revisa nomes próprios, locais e termos de lore em legendas `.ass` já traduzidas, comparando com a legenda original em inglês. Ele **não** usa este `ProvedorContexto` — tem seu próprio sistema de contextos (`ProvedorPromptRevisaoLore`, pacote `revisaoLore/contexto/**`, atualmente 9 obras calibradas). Documentação completa em [Módulo: Revisão de Lore](16-modulo-revisao-lore.md).
